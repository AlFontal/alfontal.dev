...
const sourcePostsRoot = path.join(repoRoot, 'site', 'posts');
...
function buildGeneratedFrontMatter(metadata, context) {
  ...
  return {
    ...
    section,
    notebook,
    legacyPath: `/posts/${section}/${notebook}`,
  };
}
...
async function main() {
  ...
  for (const notebookContext of notebooks) {
    ...
    if (seenLegacyPaths.has(generatedFrontMatter.legacyPath)) {
      throw new Error(`Duplicate notebook legacy path detected: ${generatedFrontMatter.legacyPath}`);
    }
    seenLegacyPaths.add(generatedFrontMatter.legacyPath);
    ...
    manifest.push({
      ...
      legacyPath: generatedFrontMatter.legacyPath,
  ...
  manifest.sort((a, b) => a.legacyPath.localeCompare(b.legacyPath));
